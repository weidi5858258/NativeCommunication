...
BpCallback::BpCallback(const sp<IBinder> &impl)
    ...
    pid_t pid = getpid();
    LOGD("BpCallback::BpCallback()  created   %p PID: %d\n", this, pid);
}

BpCallback::~BpCallback() {
    pid_t pid = getpid();
    LOGD("BpCallback::~BpCallback() destroyed %p PID: %d\n", this, pid);
}
...
int BpCallback::onRecognize(size_t len,
                            ...
                            const char *result) {
    pid_t pid = getpid();
    LOGD("BpCallback::onRecognize(6) %p PID: %d captureType: %d fileName: %s", this, pid, captureType, fileName);
    Parcel data, reply;
    ...
    LOGD("BpCallback::onRecognize(6) status: %d\n", status);
    return status;
}

int BpCallback::onRecognize(int captureType, const char *result) {
    pid_t pid = getpid();
    LOGD("BpCallback::onRecognize(2) %p PID: %d captureType: %d result: %s", this, pid, captureType, result);
    Parcel data, reply;
    data.writeInterfaceToken(ICallback::getInterfaceDescriptor());
    data.writeInt32(captureType);
    ...
    LOGD("BpCallback::onRecognize(2) status: %d\n", status);
    return status;
}

int BpCallback::onError(int errorCode) {
    pid_t pid = getpid();
    LOGD("BpCallback::onError() %p PID: %d errorCode: %d", this, pid, errorCode);
    Parcel data, reply;
    data.writeInterfaceToken(ICallback::getInterfaceDescriptor());
    data.writeInt32(errorCode);
    status_t status = remote()->transact(ICallback::ON_ERROR, data, &reply);
    LOGD("BpCallback::onError() status: %d\n", status);
    return status;
}